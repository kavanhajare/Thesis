...
public class ERDB {
    ...
    public void fetchSchemaData() {
        String countQueryStatement="select count(*) as count from erql_digital_library.query";
        ...
        for(int i=1;i<=count;i++){
            String queryDetailsStatement="select get_entity,predicate_entity,equality_attributes,range_attributes,order_type from erql_digital_library.query where query_no="+i;
            ...
            if(predicate_table.contains(entity_name)  && !equality_attribute.isEmpty()){
                ...
                String entityStatement="select entity_name from erql_digital_library.entity_attribute where attribute_name='"+equalityAttribute+"' ALLOW FILTERING";
                ...
                String equalityAttribute=range_attribute.iterator().next();
                String entityStatement="select entity_name from erql_digital_library.entity_attribute where attribute_name='"+equalityAttribute+"' ALLOW FILTERING";
                ResultSet resultSet7=session.execute(entityStatement);
            ...
            if(schemaDetails.containsKey(entity_name+"_by_"+predicate_table)){
                ...
                String schemaAttributesStatement="select attribute_name,attribute_type from erql_digital_library.entity_attribute where entity_name='"+entity_name+"' ALLOW FILTERING";
                ...
                for(Row z:resultSet2.all()) {
                    ...
                    if(z.getString("attribute_type").equals("composite")){
                        String compositeAttributesStatement="select sub_attribute_name from erql_digital_library.entity_composite_attribute where attribute_name='"+z.getString("attribute_name")+"' ALLOW FILTERING";
                ...
                if(predicate_table.contains(".")){
                    ...
                    for(int entity=0; entity<predicateEntities.length-1; entity++) {
                        ...
                        String primaryAttributesOfPredicateEntityStatement = "select primary_key from erql_digital_library.entity where entity_name='" + predicateEntities[entity] + "'";
            ...
            if(!equality_attribute.isEmpty()){
                for(String partitionAttribute:equality_attribute){
                    if(attributesSet.contains(partitionAttribute)){
                        String multivalueAttributeStatement="select multivalue from erql_digital_library.entity_attribute where attribute_name='"+partitionAttribute+"' allow filtering";
...
//                    predicateEntity =predicateEntities[predicateEntities.length-1];
            ...
            String primaryAttributesStatement="select primary_key from erql_digital_library.entity where entity_name='"+entity_name+"'";
            ResultSet resultSet4=session.execute(primaryAttributesStatement);
            Row a=resultSet4.one();
